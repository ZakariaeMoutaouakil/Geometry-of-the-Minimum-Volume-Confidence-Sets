...
def calculate_probability(p: Tuple[float, ...], x: Tuple[int, ...], factorials: Tuple[int, ...]) -> float:
    """
...
if __name__ == "__main__":
    ...
    factorial_list = tuple(factorial(i) for i in range(total + 1))

    start_time = time()  # Start time
    final_probability = calculate_probability(proba, counts, factorial_list)
